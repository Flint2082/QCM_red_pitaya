...
# copied from the opcupa examples
###
...
if __name__ == "__main__":
    ...
    QCM = server.nodes.objects.add_object(idx, "QCM")

    # Variables TO QCM sensor    
    start_measurement = QCM.add_variable(idx, "start-measurement", False)
    start_measurement.set_writable()
    reset_thickness = QCM.add_variable(idx, "reset-thickness", False)
    reset_thickness.set_writable()
    z_ratio = QCM.add_variable(idx, "z-ratio", 0.0)
    z_ratio.set_writable()
    tooling_ratio = QCM.add_variable(idx, "tooling-ratio", 0.0)
    tooling_ratio.set_writable()
    mass_density = QCM.add_variable(idx, "mass-density", 0.0)
    mass_density.set_writable()
    
    mass_sensitivity = QCM.add_variable(idx, "mass-sensitivity", 0.0)
    mass_sensitivity = QCM.add_variable(idx, "mass-sensitivity", 0.0)
    mass_sensitivity.set_writable()
    
    # Variables FROM QCM sensor
    thickness = QCM.add_variable(idx, "Thickness", 0.0)
    thickness.set_writable()
    rate = QCM.add_variable(idx, "Rate", 0.0)
    rate.set_writable()
    ...
    try:
        server.import_xml("src_server/custom_nodes.xml")
    except FileNotFoundError:
        logging.warning("custom_nodes.xml not found; skipping import.")
